...
// ============================================================
//  ✏️  SET YOUR INSTRUCTIONS IMAGE PATH HERE
// ============================================================
const INSTRUCTIONS_IMG = 'images/Instuctions.png'; // ← change this to match your filename
// ============================================================


const FLOWERS = [
  { id:0, name:'Lavender',      imgKey:'lavender',      color:'#D5CDE6', cardSrc:'images/flowers/lavender.png',      tags:['dye','food','tea'], desc:'Lavender (Lavandula angustifolia) is beloved for its calming fragrance and striking purple blooms.', uses:'Dye: Produces soft grey-blue and purple hues. Tea: A calming herbal tea. Food: Adds floral notes to shortbread, honey, and lemonade.' },
  { id:1, name:'Hibiscus',      imgKey:'hibiscus',      color:'#D39BA0', cardSrc:'images/flowers/hibiscus.png',      tags:['dye','food','tea'], desc:'Hibiscus (Hibiscus sabdariffa) is a tropical flower famous for its bold magenta-red calyces.', uses:'Dye: Deep crimson-pink and purple dyes. Tea: Tart and vibrant, rich in Vitamin C. Food: Petals used in salads, jams, and sauces.' },
  { id:2, name:'Sunflower',     imgKey:'sunflower',     color:'#EE9452', cardSrc:'images/flowers/sunflower.png',     tags:['dye','food'],       desc:'Helianthus annuus is a towering symbol of warmth and vitality, native to the Americas.', uses:'Dye: Petals yield warm yellows and golds. Food: Seeds eaten roasted, pressed into oil, or made into sunflower butter.' },
  { id:3, name:'Calendula',     imgKey:'marigold',      color:'#EE9452', cardSrc:'images/flowers/maragold.png',      tags:['dye','food','med'], desc:'Calendula officinalis, the pot marigold, has bright orange-yellow blooms used medicinally for centuries.', uses:'Dye: Yields rich oranges and yellows. Food: Petals garnish salads and soups. Medicine: Calendula cream soothes skin.' },
  { id:4, name:'Gardenia',      imgKey:'gardenia',      color:'#F5F0E0', cardSrc:'images/flowers/garenia.png',       tags:['dye','tea','med'],  desc:'Gardenia jasminoides is prized for its intensely fragrant white blossoms. Native to tropical Asia and Africa.', uses:'Dye: Fruit pods yield a bright yellow dye. Tea: Used in traditional Chinese medicine. Medicine: Reduces fever.' },
  { id:5, name:'Coreopsis',     imgKey:'coreopsis',     color:'#F0C040', cardSrc:'images/flowers/Coreopsis.png',     tags:['dye','food','tea'], desc:'Coreopsis tinctoria, the plains coreopsis, is a cheerful golden wildflower native to North America.', uses:'Dye: Produces vivid golds and rusts. Tea: Mild herbal tea with a honey-like flavour. Food: Edible petals.' },
  { id:6, name:'Weld',          imgKey:'weld',          color:'#D4D88A', cardSrc:'images/flowers/weld.png',          tags:['dye','med'],        desc:"Reseda luteola, known as weld or dyer's rocket, is one of the oldest yellow dye plants in the world.", uses:'Dye: Premier yellow dye of medieval Europe. Medicine: Historically used as a wound herb.' },
  { id:7, name:'Black Scabiosa',imgKey:'blackscabiosa', color:'#4A3A5A', cardSrc:'images/flowers/black scaboisa.png',tags:['dye','food','med'], desc:'Scabiosa atropurpurea produces deep burgundy-to-near-black blooms beloved by bees and butterflies.', uses:'Dye: Dark petals yield soft mauve tones. Medicine: Used in folk medicine. Food: Petals are edible.' },
];
...
[
  ...
  ['cinnamonroll','images/game items/desert/cinnamon roll.png'],
...
document.getElementById('card-modal-close').addEventListener('click', () => { cardModal.style.display='none'; document.getElementById('card-modal-img').src=''; });
cardModal.addEventListener('click', e => { if(e.target===cardModal){ cardModal.style.display='none'; document.getElementById('card-modal-img').src=''; }});
function showCardModal(src) { document.getElementById('card-modal-img').src=src; cardModal.style.display='flex'; }
...
function showCollectCard(imgKey, kind) {
  ...
  wrap.innerHTML = '';
  if (imgReady(imgKey)) {
    const el = new Image(); el.src = IMGS[imgKey].src;
  ...
  const tagsEl=document.getElementById('cc-tags'), descEl=document.getElementById('cc-desc'), hintEl=document.getElementById('cc-hint');
  tagsEl.innerHTML=''; descEl.style.display='none';
  if (kind==='flower') {
    const entry=FLOWER_BY_IMGKEY[imgKey];
    document.getElementById('cc-name').textContent=entry?entry.name:imgKey;
    if (entry) {
      descEl.textContent=entry.desc.slice(0,90)+'…'; descEl.style.display='block';
      entry.tags.forEach(t=>{ const s=document.createElement('span'); s.className=`tag ${TAG_CLS[t]}`; s.textContent=TAG_LABEL[t]; tagsEl.appendChild(s); });
    }
    hintEl.style.display='block';
  } else {
    document.getElementById('cc-name').textContent=DESSERT_NAMES[imgKey]||imgKey;
    const s=document.createElement('span'); s.className='tag tag-food'; s.textContent='Dessert'; tagsEl.appendChild(s);
    hintEl.style.display='none';
  }
  collectCard.style.pointerEvents='auto';
  requestAnimationFrame(()=>{ collectCard.style.opacity='1'; collectCard.style.transform='translate(-50%,-50%) scale(1)'; });
  clearTimeout(collectCardTimer);
  collectCardTimer=setTimeout(hideCollectCard, 3500);
}
function hideCollectCard() { collectCard.style.opacity='0'; collectCard.style.transform='translate(-50%,-50%) scale(0.7)'; collectCard.style.pointerEvents='none'; }
...
function showModal(f) {
  ...
  document.getElementById('modalTags').innerHTML=f.tags.map(t=>`<span class="tag ${TAG_CLS[t]}">${TAG_LABEL[t]}</span>`).join('');
...
// ════════════════════════════════════════════════════════════
//  COMPLETION SCREEN  — pixel garden tea party
// ════════════════════════════════════════════════════════════
function buildCompletionScreen() {
  ...
  // Pixel garden background canvas
  el.innerHTML = `
...
function drawPixelGarden(cv) {
  ...
  // Pixel flowers scattered in background
...
function drawTeaScene() {
  ...
  // Helper to place an image in the scene
...
function populateFlowerSummary() {
  ...
  found.forEach(f => {
    ...
    if (imgReady(f.imgKey)) {
      ...
      thumb.style.background = f.color;
    }
...
document.addEventListener('mousemove', e=>{ targetX=e.clientX; targetY=e.clientY; follower.style.opacity='1'; });
(function animFollower(){
  curX+=(targetX-curX)*0.14; curY+=(targetY-curY)*0.14;
  follower.style.left=(curX-16)+'px'; follower.style.top=(curY-16)+'px';
  requestAnimationFrame(animFollower);
})();
toggle.addEventListener('change',()=>{ follower.classList.toggle('bee-follower',!toggle.checked); follower.classList.toggle('butterfly-follower',toggle.checked); });


// ════════════════════════════════════════════════════════════
//  HOW TO PLAY MODAL — uses uploaded instruction image as overlay
// ════════════════════════════════════════════════════════════
...
howToPlayModal.innerHTML = `
  <div style="position:relative; max-width:92vw; max-height:90vh;">
    ...
    <button id="htp-close" style="
      ...
      border:2px solid #7b8c63;
      border-radius:50%; width:34px; height:34px;
      font-size:1rem; cursor:pointer; line-height:1;
      display:flex; align-items:center; justify-content:center;
      box-shadow:2px 2px 0 rgba(60,80,40,0.2);
    ">✕</button>
  </div>
`;
document.body.appendChild(howToPlayModal);
// Set image src from the constant at the top of the file
...
// Wire up How to Play button on menu
...
if (!sessionStorage.getItem('htp-seen')) {
  ...
  if (document.readyState !== 'loading') setTimeout(showHowToPlay, 400);
}
...
aboutDropdown.innerHTML = `
  <button id="about-toggle" style="
    ...
    box-shadow:2px 2px 0 rgba(123,140,99,0.15);
  ...
  <div id="about-panel" style="
    ...
    border:2px solid rgba(123,140,99,0.35);
    border-radius:14px;
    padding:16px 24px 14px;
    box-shadow:4px 4px 0 rgba(123,140,99,0.2);
    max-width:320px;
    text-align:center;
  ">
...
document.getElementById('about-toggle').addEventListener('click', () => {
  ...
  // flip up vs down
  aboutDropdown.style.bottom = isOpen ? '18px' : '18px';
});
...
function showScreen(name) {
  ...
  // Hide about dropdown when not on menu
...
document.querySelectorAll('.js-index-btn').forEach(el=>
  el.addEventListener('click',()=>{
    ...
    buildIndex(); showScreen('index');
  })
);
document.getElementById('backFromIndex').addEventListener('click',()=>showScreen(indexReturnScreen));
...
let COLLECTIBLES = [
  // ── FLOWERS (top grass band: wy 0.04–0.27) ──────────────────
  // Spread 24 flowers across full 3600px, alternating top/bottom
  ...
  // ── BUSHES — one every ~280px, alternating top/bottom ───────
  ...
  // ── DESSERTS — on the path (wy ~0.44–0.56), one every ~330px ─
...
function seedGravel(H){ gravelPebbles=[]; const pt=H*((1-PATH_RATIO)/2),ph=H*PATH_RATIO; for(let i=0;i<500;i++) gravelPebbles.push({wx:Math.random()*WORLD_W,wy:pt+4+Math.random()*(ph-8),rx:1.2+Math.random()*2.5,ry:0.7+Math.random()*1.5,rot:Math.random()*Math.PI,col:Math.random()<0.5?'#b4a494':'#cbbfaf'}); }
function seedGrassTiles(H){ grassTiles=[]; const pt=H*((1-PATH_RATIO)/2),pb=H*((1+PATH_RATIO)/2),cols=['#9db89a','#a8c4a4','#91ae8e','#b5ccb0','#8aaa87','#bdd4b8','#96b293']; for(let x=0;x<WORLD_W;x+=8){for(let y=0;y<pt;y+=8) if(Math.random()<0.7) grassTiles.push({x,y,col:cols[Math.floor(Math.random()*cols.length)]});for(let y=pb;y<H;y+=8) if(Math.random()<0.7) grassTiles.push({x,y,col:cols[Math.floor(Math.random()*cols.length)]});} }
function seedDirtTiles(H){ dirtTiles=[]; const pt=H*((1-PATH_RATIO)/2),ph=H*PATH_RATIO,cols=['#c8b090','#c0a882','#d4bc9e','#b89a78','#cbb498','#bca080']; for(let x=0;x<WORLD_W;x+=8) for(let y=pt;y<pt+ph;y+=8) if(Math.random()<0.65) dirtTiles.push({x,y,col:cols[Math.floor(Math.random()*cols.length)]}); }
...
document.getElementById('gameCanvas').addEventListener('click',e=>{
  ...
  for(const obj of COLLECTIBLES){
    ...
    if(dist<(obj.kind==='bush'?32:26)){
      ...
      // Check completion — all collected
...
function gameLoop() {
  const canvas=document.getElementById('gameCanvas');
  if(!canvas) return;
  // Keep running even if screen is hidden — just skip drawing
  ...
  ctx.fillStyle='#9db89a'; ctx.fillRect(0,0,WORLD_W,pathTop); ctx.fillRect(0,pathBot,WORLD_W,H-pathBot);
  for(const t2 of grassTiles){if(t2.x+8<visL||t2.x>visR)continue;ctx.fillStyle=t2.col;ctx.fillRect(t2.x,t2.y,8,8);}
  ctx.strokeStyle='rgba(70,100,60,0.28)';ctx.lineWidth=1;
  for(let gx=4;gx<WORLD_W;gx+=14){if(gx<visL||gx>visR)continue;const sw=Math.sin(t*0.9+gx*0.04)*2.5;ctx.beginPath();ctx.moveTo(gx,pathTop*0.85);ctx.lineTo(gx+sw,pathTop*0.45);ctx.stroke();ctx.beginPath();ctx.moveTo(gx,pathBot+(H-pathBot)*0.15);ctx.lineTo(gx+sw,pathBot+(H-pathBot)*0.55);ctx.stroke();}

  ctx.fillStyle='#c8b090';ctx.fillRect(0,pathTop,WORLD_W,pathH);
  for(const t2 of dirtTiles){if(t2.x+8<visL||t2.x>visR)continue;ctx.fillStyle=t2.col;ctx.fillRect(t2.x,t2.y,8,8);}
  ...
  for(const p of gravelPebbles){if(p.wx<visL||p.wx>visR)continue;ctx.fillStyle=p.col;ctx.beginPath();ctx.ellipse(p.wx,p.wy,p.rx,p.ry,p.rot,0,Math.PI*2);ctx.fill();}
  ...
  ctx.fillStyle='rgba(100,120,80,0.18)';ctx.fillRect(0,0,40,H);ctx.fillRect(WORLD_W-40,0,40,H);
  ctx.restore();
...
//  DRAW HELPERS
// ════════════════════════════════════════════════════════════
function drawSparkles(ctx,cx,cy,t,phase){ctx.save();for(let i=0;i<6;i++){const a=(i/6)*Math.PI*2+t*0.55+phase,d=15+Math.sin(t*2.2+i*0.9)*7,px=Math.round(cx+Math.cos(a)*d),py=Math.round(cy+Math.sin(a)*d),alpha=(Math.sin(t*3.0+i+phase)+1)/2;ctx.globalAlpha=alpha*0.85;ctx.fillStyle='#ffffff';ctx.fillRect(px-1,py,3,1);ctx.fillRect(px,py-1,1,3);ctx.fillStyle='#FFE566';ctx.fillRect(px,py,1,1);}ctx.restore();}
function hoverGlow(ctx,wx,wy,r,col){ctx.save();ctx.globalAlpha=0.22;ctx.fillStyle=col;ctx.beginPath();ctx.arc(wx,wy,r+10,0,Math.PI*2);ctx.fill();ctx.restore();}
function drawCollected(ctx,wx,wy,t){ctx.save();ctx.globalAlpha=0.45;for(let i=0;i<5;i++){const a=(i/5)*Math.PI*2+t*1.2,r=10+Math.sin(t*2+i)*3;ctx.fillStyle='#E0B451';ctx.beginPath();ctx.arc(wx+Math.cos(a)*r,wy+Math.sin(a)*r,2,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=0.6;ctx.fillStyle='#4a5c35';ctx.font='10px Lora,serif';ctx.textAlign='center';ctx.fillText('✓',wx,wy+4);ctx.restore();}
function drawFlowerSpot(ctx,wx,wy,imgKey,t,hover){ctx.save();if(hover)hoverGlow(ctx,wx,wy,26,'#D5CDE6');if(!drawImgCentered(ctx,imgKey,wx,wy-2,44)){ctx.strokeStyle='rgba(100,130,70,0.5)';ctx.lineWidth=2;ctx.setLineDash([3,3]);ctx.beginPath();ctx.moveTo(wx,wy+22);ctx.lineTo(wx,wy-2);ctx.stroke();ctx.setLineDash([]);for(let i=0;i<4;i++){const a=(i/4)*Math.PI*2;ctx.fillStyle='rgba(213,205,230,0.55)';ctx.beginPath();ctx.ellipse(wx+Math.cos(a)*9,wy-2+Math.sin(a)*9,6,10,a,0,Math.PI*2);ctx.fill();}ctx.fillStyle='rgba(224,180,81,0.6)';ctx.beginPath();ctx.arc(wx,wy-2,5,0,Math.PI*2);ctx.fill();}if(SPARKLE_FLOWERS.has(imgKey))drawSparkles(ctx,wx,wy-2,t,(wx*0.013)%(Math.PI*2));ctx.restore();}
function drawBushSpot(ctx,wx,wy,imgKey,hover){ctx.save();if(hover)hoverGlow(ctx,wx,wy,30,'#9db89a');if(!drawImgCentered(ctx,imgKey,wx,wy,56)){[[-16,6],[16,6],[0,-6]].forEach(([ox,oy],i)=>{ctx.fillStyle=['rgba(110,148,85,0.45)','rgba(88,124,65,0.40)','rgba(130,162,100,0.42)'][i];ctx.beginPath();ctx.arc(wx+ox,wy+oy,22,0,Math.PI*2);ctx.fill();});}ctx.restore();}
function drawDessert(ctx,wx,wy,imgKey,hover){ctx.save();if(hover)hoverGlow(ctx,wx,wy,24,'#f5d5b0');if(!drawImgCentered(ctx,imgKey,wx,wy,42)){ctx.fillStyle='#e8d4c0';ctx.beginPath();ctx.arc(wx,wy,18,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#c0a080';ctx.lineWidth=1.5;ctx.stroke();}ctx.restore();}
function drawArrow(ctx,x,y,dir){ctx.save();ctx.fillStyle='rgba(100,125,75,0.6)';ctx.beginPath();const s=11;if(dir==='left'){ctx.moveTo(x+s,y-s);ctx.lineTo(x-s,y);ctx.lineTo(x+s,y+s);}else{ctx.moveTo(x-s,y-s);ctx.lineTo(x+s,y);ctx.lineTo(x-s,y+s);}ctx.closePath();ctx.fill();ctx.restore();}
function spawnBurst(x,y,kind){const col={flower:'#D5CDE6',bush:'#b5ccb0',dessert:'#f5d5b0'}[kind]||'#E0B451';for(let i=0;i<10;i++){const p=document.createElement('div');p.className='pollen-burst';const angle=(i/10)*Math.PI*2,dist=28+Math.random()*22;p.style.cssText=`left:${x-4}px;top:${y-4}px;background:${col};`;document.body.appendChild(p);requestAnimationFrame(()=>{p.style.transform=`translate(${Math.cos(angle)*dist}px,${Math.sin(angle)*dist}px) scale(.2)`;p.style.opacity='0';});p.addEventListener('transitionend',()=>p.remove(),{once:true});}}


// ============================================================
//  VINE TOOLTIPS
// ============================================================
const vineFlowerData={'flower-wisteria':{name:'Wisteria',fact:'Produces soft lilac dyes and was historically woven into garlands.'},'flower-foxglove':{name:'Foxglove',fact:'Source of digitalis, a powerful heart medicine. Not edible!'},'flower-lilac':{name:'Lilac',fact:'Used in perfumery and produces light purple dyes.'},'flower-blush':{name:'Blush Peony',fact:'Petals are edible and used in teas and floral salads.'},'flower-cornflower':{name:'Cornflower',fact:'Produces a vivid blue dye and petals are edible in salads.'}};
const tt=document.createElement('div');tt.id='vine-tooltip';tt.style.cssText='position:fixed;z-index:99998;background:rgba(255,255,255,.92);border:2px solid var(--deep-sage);border-radius:10px;padding:6px 12px;font-family:Lora,serif;font-size:.78rem;color:var(--deep-sage);pointer-events:none;opacity:0;transition:opacity .2s;max-width:160px;line-height:1.4;box-shadow:2px 3px 0 rgba(123,140,99,.25);';
document.body.appendChild(tt);
document.querySelectorAll('.pixel-flower').forEach(el=>{const cls=[...el.classList].find(c=>vineFlowerData[c]);if(!cls)return;const{name,fact}=vineFlowerData[cls];el.addEventListener('mouseenter',e=>{tt.innerHTML=`<strong>${name}</strong><br>${fact}`;tt.style.opacity='1';positionTooltip(e);});el.addEventListener('mousemove',positionTooltip);el.addEventListener('mouseleave',()=>tt.style.opacity='0');});
function positionTooltip(e){tt.style.left=Math.min(e.clientX+24,window.innerWidth-tt.offsetWidth-10)+'px';tt.style.top=Math.max(10,Math.min(e.clientY-10,window.innerHeight-tt.offsetHeight-10))+'px';}

// ============================================================
//  UTILITIES
// ============================================================
function checkPollinatorCollision(x,y){for(const f of document.querySelectorAll('.pixel-flower')){const r=f.getBoundingClientRect();if(x>=r.left-16&&x<=r.right+16&&y>=r.top-16&&y<=r.bottom+16)return f;}return null;}
window.checkPollinatorCollision=checkPollinatorCollision;
function triggerPollinationEffect(flowerEl){const rect=flowerEl.getBoundingClientRect();const ox=rect.left+rect.width/2,oy=rect.top+rect.height/2;for(let i=0;i<8;i++){const p=document.createElement('div'),a=(i/8)*Math.PI*2,d=28+Math.random()*20;p.style.cssText=`position:fixed;left:${ox-4}px;top:${oy-4}px;width:8px;height:8px;background:var(--pollen);border-radius:50%;pointer-events:none;z-index:99997;transition:transform .6s ease-out,opacity .6s ease-out;`;document.body.appendChild(p);requestAnimationFrame(()=>{p.style.transform=`translate(${Math.cos(a)*d}px,${Math.sin(a)*d}px) scale(.3)`;p.style.opacity='0';});p.addEventListener('transitionend',()=>p.remove(),{once:true});}}
